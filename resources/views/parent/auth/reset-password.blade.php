...
<div  class="card-header no-border">
    ...
    <h6 class="card-subtitle line-on-side text-muted text-xs-center font-small-3 pt-2"><span>Parent Reset Password</span></h6>
		@if(session('success'))
		...
		$.ajax({
                    ...
                    success:function(response){
						...
						if(response) {
							...
							window.location = '{{ route("parent_login_page") }}';
						}
                    },
					error:function(XMLHttpRequest,textStatus,errorThrown){
						...
						if (responseCode == 419) {
							$("#error").html(response['message']);
							window.location = '{{ route("parent_forget_password") }}';
						}
						else {
							$("#error").html(response['message']);
						}
