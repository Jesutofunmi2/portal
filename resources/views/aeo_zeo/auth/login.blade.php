...
<div class="card-header no-border">
    ...
    <h6 class="card-subtitle line-on-side text-muted text-xs-center font-small-3 pt-2"><span>AOE / ZEO Login</span></h6>
		@if(session('success'))
...
<div class="card-body collapse in">
	<div id="login-form" class="card-block">
			...
			<br>
			<a href="{{ route('aeo_zeo_forget_password') }}">Forget Password?</a>
  ...
  <script type="text/javascript">
		...
		$.ajax({
					...
					error:function(XMLHttpRequest,textStatus,errorThrown){
						...
						if (responseCode == 400){
							...
							window.location = '{{ route("aeo_zeoUpdateEmail") }}';
						}
	...
	function loginUser(){
		...
		$.ajax({
                    url: "{{ route('aeo_zeo_login_page_post') }}",
                    type:"POST",
                    data:{
                        username:username,
                        password:password,
