...
<div  class="card-header no-border">
    ...
    <h6 class="card-subtitle line-on-side text-muted text-xs-center font-small-3 pt-2"><span>Ministry Login</span></h6>
		@if(session('success'))
...
<div class="card-body collapse in">
	<div id="login-form" class="card-block">
			...
			<button type="button" style="background-color: #{{ env('APP_COLOR')}}; color:#fff" onclick="sendOTP()" id="login_btn" class="btn btn-lg btn-block"><i class="icon-unlock2"></i> Login</button>
			<br>
			<a href="{{ route('ministry_forget_password') }}">Forget Password?</a>
			<br>
	...
	<div id="otp-form" class="hidden">
		...
		<button type="button"  onclick="loginUser()" id="login_otp_btn" style="background-color: #{{ env('APP_COLOR')}}; color:#fff" class="btn btn-lg btn-block"><i class="icon-unlock2"></i> Continue</button>
...
<script type="text/javascript">
	function sendOTP(){
		...
		$.ajax({
					...
					error:function(XMLHttpRequest,textStatus,errorThrown){
						...
						if (responseCode == 400){
							$("#error").html(response['message'] + ", you will be redirect to email verification page...");
							window.location = '{{ route("updateEmail") }}';
						}
        			}
                });
	}
	...
	function loginUser(){
		...
		$.ajax({
                    url: "{{ route('ministry_login_page_post') }}",
                    type:"POST",
                    data:{
                        username:username,
                        password:password,
