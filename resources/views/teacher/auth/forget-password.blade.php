...
<div  class="card-header no-border">
    ...
    <h6 class="card-subtitle line-on-side text-muted text-xs-center font-small-3 pt-2"><span>Teacher Reset Password</span></h6>
		@if(session('success'))
...
<div class="card-body collapse in">
	<div id="login-form" class="card-block">
			...
			<fieldset class="form-group position-relative has-icon-left mb-0">
				<input type="text" class="form-control form-control-lg input-lg" id="username" placeholder="Enter your Staff No or Email" name="username" value="" required autofocus>
...
<script type="text/javascript">
	function sendMail(){
		...
		$.ajax({
                    url: "{{ route('teacher_forget_password') }}",
                    type:"POST",
                    data:{
                        username : username,
                        _token : _token,
					...
					error:function(XMLHttpRequest,textStatus,errorThrown){
						...
						if (responseCode == 419) {
							$("#error").html(response['message']);
							window.location = '{{ route("teacher_forget_password") }}';
						}
						else {
							$("#error").html(response['message']);
						}
