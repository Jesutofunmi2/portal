...
<div class="card-header no-border">
    ...
    <h6 class="card-subtitle line-on-side text-muted text-xs-center font-small-3 pt-2"><span>Teacher Login</span></h6>
		@if(session('success'))
...
<div class="card-body collapse in">
	<div id="login-form" class="card-block">
			...
			<fieldset class="form-group position-relative has-icon-left mb-0">
				<input type="text" class="form-control form-control-lg input-lg" id="staff_no" placeholder="Your Staff No" name="staff_no" value="{{old('staff_no') }}" required autofocus>
			...
			<br>
			<a href="{{ route('teacher_forget_password') }}">Forget Password?</a>
	...
	<div id="otp-form" class="hidden">
		...
		<button type="button" style="background-color: #{{ env('APP_COLOR')}}; color:#fff" onclick="loginUser()" id="login_otp_btn" class="btn btn-lg btn-block"><i class="icon-unlock2"></i> Continue</button>
  ...
  <script type="text/javascript">
	function sendOTP(){
		...
		if(staff_no=='' || password=='') {
			$("#error").html('Staff No and Password can not be empty');
			return;
		}
	...
	function loginUser(){
		...
		$.ajax({
                    url: "{{ route('teacher_login_page_post') }}",
                    type:"POST",
                    data:{
                        staff_id:staff_no,
                        password:password,
	...
	function apiLogin(){
		...
		$.ajax({
                    ...
                    if(response) {
						...
						localStorage.setItem('username', staff_no);
