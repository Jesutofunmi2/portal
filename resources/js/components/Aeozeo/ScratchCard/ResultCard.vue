    ...
    <div class="col-xs-12">
        <div class="card">
            <div class="card-header">
                <h4 class="card-title">Result Scratch Card</h4>
						...
						<div class="row" style="background:#eee; border:1px solid #ccc; border-radius:5px; padding:5px; text-align:center;">
							...
							<div class="col-md-6" >
									<div class="input-group col-xs-12">
										...
										<select type="text" id="" v-model="card_quantity" class="form-control" name="state" data-toggle="tooltip" data-trigger="hover" data-placement="top" data-title="Select Quantity">
		...
		methods : {
           generateCards(){
			   ...
			   this.$loading(true)
                ...
                axios.post('/api/ministry/result/scratch-card/generate',data)
				.then(response => {
                    if(response) {
						this.$loading(false)
		   ...
		   toggleAll(){
			   ...
			   this.$confirm("Are you sure you want delete all Scratch Card ?","Delete Scratch Card",'warning').then(() => {
                ...
                this.$loading(true)
                axios.post('/api/ministry/result/scratch-card/delete/all',{'ids' : ids})
                .then((response) => {
                    this.$loading(false);
		   ...
		   getCards(page = 1){
			   ...
			   let url = '/api/ministry/result/scratch-card/view?page='+page+'&per_page='+this.filter.per_page

			   if((this.filter.category == 'available') || (this.filter.category == 'used')) {
				   url = '/api/ministry/result/scratch-card/view?page='+page+'&per_page='+this.filter.per_page+'&category='+this.filter.category
			   }
		   ...
		   toggle(id){
                ...
                axios.delete('/api/ministry/result/scratch-card/'+id+'/delete')
                .then((response) => {
                    this.flashMessage.success({
                            title: 'Successful',
                            message: response.data.data.message,
