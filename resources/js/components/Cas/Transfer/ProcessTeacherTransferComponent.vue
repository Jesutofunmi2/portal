...
<script>
    ...
    export default {
		...
		methods : {
			...
			getTeacher() {
				this.$loading(true);
				axios.get(`/api/ministry/transfer/processteacher/${this.teacherId}`)
				.then((res) => {
            ...
            processTeacherTransfer() {
                ...
                axios.post(`/api/ministry/transfer/processteacher/${this.teacherId}/update`, data)
				.then(response => {
                    if(response) {
						this.$loading(false)
