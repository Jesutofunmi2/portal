    ...
    <div class="col-xs-12">
        <div class="card">
            <div class="card-header">
                <h4 class="card-title">List Primary School</h4>
            ...
            <div class="card-body collapse in">
                   ...
                   <div class="col-md-6">
                             ...
                             <fieldset style="width:100%;" class="row py-2">
                                <div class="input-group col-xs-12">
                                    ...
                                    <span class="input-group-btn" id="button-addon2">
                                        <button @click="searchByLga" class="btn btn-lg btn-success border-grey border-lighten-1" type="submit"><i class="icon-ios-search-strong"></i></button>
                                    </span>
                                </div>
                            </fieldset>
                ...
                <div class="table-responsive">
                      <div v-if="pagination">
                          <button @click="getSchools(pagination.prev)" :class="pagination.prev ? '' : 'disabled'" class="btn btn-success"> &lt;&lt;  Prev </button> {{ meta.current_page }} of {{ meta.last_page }} <button @click="getSchools(pagination.next)" :class="pagination.next ? '' : 'disabled'" class="btn btn-success"> Next &gt;&gt;  </button>
                      </div>
                    <table class="table table-striped mb-0">
                        ...
                        <tbody>
                            <tr v-for="(school , index) in schools" :key="index">
                                ...
                                <td>
                                    <button @click="editSchool(school.id)"  class="btn btn-primary"><i class="icon-edit"></i> Edit </button>
                                </td>
                                 <td>
                                   <button @click="deleteSchool(school.id)"  class="btn btn-danger"><i class="icon-trash4"></i> Delete</button>
                                </td>
                            </tr>
...
<script>
    ...
    export default {
		...
		methods : {
            ...
            getSchools($url = '/api/ministry/school/primary/view'){
                if($url==null) return ;
				this.$loading(true);
				axios.get($url)
				.then((res) => {
            ...
            searchByTerm(){
                if(this.query == null || this.query=='') return ;
                 this.$loading(true);
				axios.get('/api/ministry/school/primary/view?query='+this.query)
				.then((res) => {
					this.schools = res.data.data,
					this.$loading(false)
				})
            ...
            searchByLga(){
                if(this.lga_id == null || this.lga_id=='') return ;
                 this.$loading(true);
				axios.get('/api/ministry/school/primary/view?lga_id='+this.lga_id)
				.then((res) => {
					this.schools = res.data.data,
					this.$loading(false)
				})
            ...
            deleteSchool(id){
                ...
                this.$loading(true)
                axios.delete('/api/ministry/school/primary/'+id+'/delete')
                .then((response) => {
                    this.$loading(false);
