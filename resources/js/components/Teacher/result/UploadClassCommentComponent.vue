						...
						<form class="form" novalidate @submit.prevent="uploadComments">
							<div class="form-body">
                                ...
                                <div class="form-group col-md-6">
									...
									<select id="issueinput12" v-model="comment" @change="validationErrors.comment=null" :class="{'border-danger':validationErrors.comment}" class="form-control" name="comment" data-toggle="tooltip" data-trigger="hover" data-placement="top" data-title="Comment Type" required readonly>
                                        <option selected value="comment_teacher">Teacher</option>
									</select>
    ...
    methods: {
        ...
        downloadFile() {
            if (!this.session || !this.class_id || !this.classarm_id || !this.term || !this.comment) {
                ...
                axios({
                    url: '/api/teacher/result/downloadcommtemplate',
                    method: 'GET',
                    params: params,
                    responseType: 'blob',
                }).then((response) => {
                        ...
                        let fileName = `StudentResultCommentTemplate_${this.class_name}.xls`
        ...
        uploadComments() {
            ...
            axios.post('/api/teacher/result/uploadcomments', data)
            .then(response => {
                if(response) {
                    this.$loading(false)
