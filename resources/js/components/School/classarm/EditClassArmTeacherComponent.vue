		...
		<div class="col-md-12">
			<div class="card">
				<div class="card-header">
					<h4 class="card-title" id="basic-layout-tooltip">Edit Class Arm Teachers for <span v-if="classarm.classes !== undefined">{{ classarm.classes.class_name }}</span> <span v-if="classarm.class_arm !== undefined">{{ classarm.class_arm }}</span> </h4>
				...
				<div class="card-body collapse in">
					<div class="card-block">
						...
						<form class="form" @submit.prevent="updateArmTeachers">
							...
							<div class="form-body">
								...
								<div class="form-group col-md-6">
									...
									<select id="issueinput12" v-model="teachID" @change="validationErrors.teacher_id=null" :class="{'border-danger':validationErrors.teacher_id}" class="form-control" name="teacher_id" data-toggle="tooltip" data-trigger="hover" data-placement="top" data-title="Teacher" required>
...
<script>
    ...
    export default {
		...
		methods : {
			getTeachers() {
				this.$loading(true)
				axios.get('/api/school/teacher/view/all')
				.then((res) => {
			...
			getArmTeachers() {
				this.$loading(true)
				axios.get(`/api/school/classarm/teacher/${this.classArmID}/edit`)
				.then((res) => {
					this.classarm = res.data.data
					let teachers = this.classarm.teachers
					
					if (Array.isArray(teachers) && teachers.length > 0) {
						this.teachID = teachers[0].id;
						this.session = teachers[0].pivot.session;
					}
            ...
            updateArmTeachers() {
				...
				data.append('teacher_id', this.teachID);
				data.append('session', this.session);
				
				this.$loading(true)

                axios.post(`/api/school/classarm/teacher/${this.classArmID}/update`, data)
				.then(response => {
                    if(response) {
						this.$loading(false)
