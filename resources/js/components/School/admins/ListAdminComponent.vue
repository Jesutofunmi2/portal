    ...
    <div class="col-xs-12">
        <div class="card">
            <div class="card-header">
                <h4 class="card-title">List Admin Users</h4>
            ...
            <div class="card-body collapse in">
                ...
                <div class="table-responsive">
                      <div v-if="pagination">
                          <button @click="getAdmins(pagination.prev)" :class="pagination.prev ? '' : 'disabled'" class="btn btn-success"> &lt;&lt;  Prev </button> {{ meta.current_page }} of {{ meta.last_page }} <button @click="getAdmins(pagination.next)" :class="pagination.next ? '' : 'disabled'" class="btn btn-success"> Next &gt;&gt;  </button>
                      </div>
...
<script>
    ...
    export default {
		...
		methods : {
            getAdmins($url = '/api/school/admin/view'){
                if($url==null) return ;
				this.$loading(true);
				axios.get($url)
				.then((res) => {
            ...
            searchByTerm(){
                if(this.query == null || this.query=='') return ;
                 this.$loading(true);
				axios.get(`/api/school/admin/view?query=${this.query}`)
				.then((res) => {
					this.admins = res.data.data,
                    this.pagination = res.data.links,
                    this.meta = res.data.meta,
            ...
            editAdmin(id) {
                this.$confirm("Are you sure you want to edit this Admin?","Edit Admin",'question').then(() => {
                this.$router.push({
                        name: 'edit-admin',
                        params: { adminID: id }
                        });
            ...
            deleteAdmin(id){
                this.$confirm("Are you sure you want delete this Admin?","Delete Admin",'warning').then(() => {
                    this.$loading(true)
                    axios.delete(`/api/school/admin/${id}/delete`)
                    .then((response) => {
                        this.$loading(false);
