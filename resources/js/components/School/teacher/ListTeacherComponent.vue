                            ...
                            <table class="table table-striped mb-0">
                                ...
                                <tbody>
                                    <tr v-for="(teacher, index) in teachers" :key="index">
                                        ...
                                        <td><img :src="baseUrl+teacher.passport" alt="Passport" style="max-height:100px; max-width:100px"></td>
...
<script>
    ...
    export default {
		...
		methods : {
			getTeachers($url = '/api/school/teacher/view'){
                if($url==null) return ;
				this.$loading(true);
				axios.get($url)
				.then((res) => {
            ...
            searchByTerm() {
                if(this.query == null || this.query == '') return ;
                 this.$loading(true);
				axios.get(`/api/school/teacher/view?query=${this.query}`)
				.then((res) => {
					this.teachers = res.data.data,
                    this.pagination = res.data.links,
                    this.meta = res.data.meta,
            ...
            editTeacher(id){
                this.$confirm("Are you sure you want to edit this teacher?","Edit Teacher",'question').then(() => {
                this.$router.push({
                        name: 'edit-teacher',
                        params: { teacherID: id }
                        });
            ...
            deleteTeacher(id){
                this.$confirm("Are you sure you want delete this teacher?","Delete Teacher",'warning').then(() => {
                this.$loading(true)
                axios.delete(`/api/school/teacher/${id}/delete`)
                .then((response) => {
                    this.$loading(false);
