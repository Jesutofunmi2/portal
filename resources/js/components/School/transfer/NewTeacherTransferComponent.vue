            ...
            <div class="col-xs-12">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title">Select Teacher For Transfer</h4>
                            ...
                            <table class="table table-striped mb-0">
                                ...
                                <tbody>
                                    <tr v-for="(teacher, index) in teachers" :key="index">
                                        ...
                                        <td>
                                            <div v-for="(subj, ind) in teacher.subjects" :key="ind">
                                                {{ subj.subject_name }}
                                            </div>
                                        </td>
                                        <td>
                                            <button @click="transferTeacher(teacher.id)" class="btn btn-primary"> Process Transfer </button>
                                        </td>
                                    </tr>
...
<script>
    ...
    export default {
		...
		methods : {
			getTeachers($url = '/api/school/transfer/newteacher') {
                if($url==null) return ;
				this.$loading(true);
				axios.get($url)
				.then((res) => {
            ...
            searchByTerm() {
                if(this.query == null || this.query == '') return ;
                 this.$loading(true);
				axios.get(`/api/school/transfer/newteacher?query=${this.query}`)
				.then((res) => {
					this.teachers = res.data.data,
                    this.pagination = res.data.links,
                    this.meta = res.data.meta,
            ...
            transferTeacher(id){
                this.$confirm("Do you want to process a transfer for this teacher?","Process Teacher Transfer",'question').then(() => {
                this.$router.push({
                        name: 'process-teacher-transfer',
                        params: { teacherID: id }
                        });
                });
            },
