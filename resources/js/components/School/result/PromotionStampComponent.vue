            ...
            <div class="col-xs-12">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title">Promotion Stamp For Students Results</h4>
                    ...
                    <div class="card-body collapse in">
                            ...
                            <form class="form" novalidate @submit.prevent="getStudents">
                                ...
                                <div class="form-body">
                                    ...
                                    <div class="form-group col-md-3">
                                        ...
                                        <select type="text" id="issueinput4" v-model="term" @change="validationErrors.term=null" :class="{'border-danger':validationErrors.term}" class="form-control" name="term" data-toggle="tooltip" data-trigger="hover" data-placement="top" data-title="Term" readonly required>
                                            <option selected value="Third">Third Term</option>
                                        </select>
                        ...
                        <div v-if="this.students.length > 0" class="table-responsive">
                            <div class="mb-1 ml-1">
                                <button class="btn btn-success mr-1" @click="stampResultAll('promoted')">All Promoted</button>
                                <button class="btn btn-primary mr-1" @click="stampResultAll('promoted_on_trial')">All Promoted On Trial</button>
                                <button class="btn btn-warning mr-1" @click="stampResultAll('repeat')">All To Repeat</button>
                                <button class="btn btn-danger" @click="stampResultAll('withdraw')">All Advice To Withdraw</button>                                
                            </div>
                            ...
                            <table class="table table-striped mb-0">
                                ...
                                <tbody>
                                    <tr v-for="(student , index) in students" :key="index">
                                        <th scope="row" width="2%">{{ index+1 }}</th>
                                        <td width="20%">
                                            {{ student.surname }} 
                                            {{ student.firstname }}
                                            {{ student.middlename }}
                                        </td>
                                        <td width="8%">{{ student.regnum }}</td>
                                        <td><img :src="baseUrl+student.passport" alt="Passport" style="max-height:100px; max-width:100px"></td>
                                    
                                        <td width="60%">
                                            <div class="form-check-inline">
                                                <label class="form-check-label">
                                                    <input type="radio" @change="stampResult(student.id, 'promoted')" class="form-check-input promoteclass" :name="`stamp${student.id}`"> Promoted
                                                </label>
                                            </div>
                                            <div class="form-check-inline">
                                                <label class="form-check-label">
                                                    <input type="radio" @change="stampResult(student.id, 'promoted_on_trial')" class="form-check-input promoteclass" :name="`stamp${student.id}`"> Promoted On Trial
                                                </label>
                                            </div>
                                            <div class="form-check-inline">
                                                <label class="form-check-label">
                                                    <input type="radio" @change="stampResult(student.
                                                    id, 'repeat')" class="form-check-input promoteclass" :name="`stamp${student.id}`"> Repeat
                                                </label>
                                            </div>
                                            <div class="form-check-inline">
                                                <label class="form-check-label">
                                                    <input type="radio" @change="stampResult(student.
                                                    id, 'withdraw')" class="form-check-input promoteclass" :name="`stamp${student.id}`"> Withdraw
                                                </label>
                                            </div>
                                        </td>
                                    </tr>
    ...
    export default {
		...
		methods : {
            ...
            stampResult(id, type) {
                ...
                axios.post('/api/school/result/promotionstamp', data)
				.then(response => {
                    if(response) {
						this.$loading(false)
            ...
            stampResultAll(type) {
                ...
                let promoteInputs = document.querySelectorAll('.promoteclass')
                
                promoteInputs.forEach(input => {
                    input.checked = false
                })

                this.$loading(true)
                
                axios.post('/api/school/result/promotionstampall', data)
				.then(response => {
                    if(response) {
						this.$loading(false)
                        this.flashMessage.success({
