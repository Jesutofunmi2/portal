		...
		<div class="col-md-12 ">
			<div class="card">
				<div class="card-header">
					<h4 class="card-title" id="basic-layout-tooltip">Batch Upload Comments</h4>
					...
					<div class="card-block">
                        ...
                        <div class="mb-2">
                            <button class="btn btn-primary ml-1" @click="downloadFile">
                                <i class="icon-download3"></i> DownLoad Comments Template
                            </button>
                        </div>

						<form class="form" novalidate @submit.prevent="uploadComments">
							<div class="form-body">

                                <div class="form-group pl-1 pr-1">
									<label for="issueinput12">Comment Type</label>
									<select id="issueinput12" v-model="comment" @change="validationErrors.comment=null" :class="{'border-danger':validationErrors.comment}" class="form-control" name="comment" data-toggle="tooltip" data-trigger="hover" data-placement="top" data-title="Comment Type" required>
										<option value="">Select Comment Type</option>
                                        <option value="comment_teacher">Teacher</option>
                                        <option value="comment_house">House Master</option>
                                        <option value="comment_guard">Guardian/Counsellor</option>
                                        <option value="comment_principal">Principal</option>
									</select>
									<span  v-if="validationErrors.comment" :class="['label text-danger']">{{ validationErrors.comment[0] }}</span>
								</div>
...
export default {
    ...
    methods: {
        ...
        downloadFile() {
            if (!this.session || !this.class_id || !this.classarm_id || !this.term || !this.comment) {
                this.flashMessage.error({title: 'Incomplete Data Supplied', 
                        message: 'Please select a Class, a Class Arm, a Session, a Term, and a Comment Type',
                        time: 15000, });
            } else {
                ...
                axios({
                    url: '/api/school/result/downloadcommtemplate',
                    method: 'GET',
                    params: params,
                    responseType: 'blob',
                }).then((response) => {
                        ...
                        let fileName = `StudentResultCommentTemplate_${className}.xls`
        ...
        uploadComments() {
            ...
            axios.post('/api/school/result/uploadcomments', data)
            .then(response => {
                if(response) {
                    this.$loading(false)
