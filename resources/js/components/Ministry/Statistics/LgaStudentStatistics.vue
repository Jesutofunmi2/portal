    ...
    <div class="col-xs-12">
        <div class="card">
            <div class="card-header">
                <h4 class="card-title">L.G.A. Student Statistics</h4>
            	...
            	<div class="card-block">
					<div class="row">
						...
						<div class="col-md-12" v-if="student_data">
							<div class="col-md-12">
								<div class="">
									<h4><strong> Student Statistic for {{lga_name}}'s LGA </strong></h4>
								</div>
							</div>
							...
							<div class="table-responsive">
								...
								<table class="table table-striped mb-0">
									...
									<tbody>
										<tr v-for="(school , index) in student_data" :key="index">
											<th scope="row">{{ index+1 }}</th>
											<td>{{ school.name }}</td>
											<td>
												<div class="row" v-if="school.classes">
													<div v-for="class_data, index in school.classes" :key="index" class="col-md-2" style="border:1px solid #ccc;">
														<h5>{{class_data.class_name}}</h5>
														<strong>Male: {{class_data.male_students}}</strong>
														<strong>Female: {{class_data.female_students}}</strong>
														<strong>Unknown: {{class_data.unknown_gender}}</strong>
														<strong>Total: {{class_data.total_students}}</strong>
													</div>
												</div>
												<strong>Total Male: {{school.total_male}} &nbsp;</strong>
												<strong>Total Female: {{school.total_female}} &nbsp;</strong>
												<strong>Total Unknown: {{school.total_unknown}} &nbsp;</strong>
												<strong>Overall Total: {{school.total_student}} &nbsp;</strong>
											</td>
										</tr>
						...
						<!--///////////////////////////////////////////-->
						<div class="col-md-12" v-if="student_data_all">
							...
							<div class="table-responsive">
								...
								<table class="table table-striped mb-0">
									...
									<tbody>
										<tr v-for="(school , index) in student_data_all" :key="index">
											<th scope="row">{{ index+1 }}</th>
											<td>{{ school.lga_name }}</td>
											<td>
												<div class="row" v-if="school.class_data">
													<div v-for="class_data, index in school.class_data" :key="index" class="col-md-2" style="border:1px solid #ccc;">
														<h5>{{class_data.class_name}}</h5>
														<strong>Male: {{class_data.male_students}}</strong>
														<strong>Female: {{class_data.female_students}}</strong>
														<strong>Unknown: {{class_data.unknown_gender}}</strong>
		...
		methods : {
           ...
           getState(){
			...
			getStudentData(){
				...
				this.$loading(true);
				this.student_data_all = null;
		
				axios.get('/api/ministry/lga/student-statistics?session='+this.s_session+'&lga_id='+this.lga_id)
				.then((res) => {
			...
			getStudentDataAll(){
				...
				axios.get('/api/ministry/lga/student-statistics/all?session='+this.s_session)
				.then((res) => {
					this.student_data_all = res.data.data,
					this.url = res.data.url,
					this.$loading(false);
				})
				.catch((error) => {
