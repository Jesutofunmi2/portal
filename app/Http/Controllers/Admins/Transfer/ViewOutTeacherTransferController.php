...
class ViewOutTeacherTransferController extends Controller
{
    //
    protected $admin;
    ...
    {
        if($this->permissionDeny('outward-teacher-transfer-history')){
        ...
        ->select('transfers_teacher.id as id', 'transfers_teacher.teacher_id as teacher_id', 'transfers_teacher.former_staff_no as former_staff_no', 'transfers_teacher.new_staff_no as new_staff_no', 'schools.name as new_school', 'sch.name as former_school', 'teachers.surname as surname', 'teachers.firstname as firstname', 'teachers.middlename as middlename', 'transfers_teacher.transfer_status as status', 'transfers_teacher.session as session', 'transfers_teacher.term as term', 'transfers_teacher.reason_for_transfer as reason')
        ->join('transfers_teacher', 'teachers.id', '=', 'transfers_teacher.teacher_id')
        ->join('schools', 'transfers_teacher.new_school', '=', 'schools.id')
        ->join('schools as sch', 'transfers_teacher.former_school', '=', 'sch.id')
