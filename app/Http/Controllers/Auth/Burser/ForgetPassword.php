...
class ForgetPassword extends Controller
{
    public function forget(Request $request)
    {
        if($request->isMethod('post')) {
           $user =  Burser::whereUsername($request->username)->first();
           ...
           if(is_null($user->email)) {
            ...
            $link = route('burser_reset_password').'?email='.$email.'&token='.$token.'&expire='.$expire_date;
        ...
        return view('burser.auth.forget-password');
    }
    ...
    public function reset(Request $request)
    {
        ...
        if ( ! $validation->passes()) {
            return redirect()->route('burser_forget_password')->withErrors($validation);
        }

        $user =  Burser::whereEmail($request->email)->first();

        if(is_null($user)) {
            return redirect()->route('burser_forget_password')->withErrors('User not found');
        }

        $decrypted = Encryption::decrypter($request->token);

        if ($user->fullname != $decrypted){
            return redirect()->route('burser_forget_password')->withErrors("Invalid token, please try again");
        }
        ...
        if ($now > $expire_date){
            return redirect()->route('burser_forget_password')->withErrors("Link expired, please try again");
        }

        return view('burser.auth.reset-password')->with([
            'username' => $user->username,
            'email' => $user->email,
    ...
    public function set(Request $request)
    {
        ...
        $user = Burser::whereEmail($request->email)->update(['password' => Hash::make($request->password)]);
