...
class AuthController extends Controller
{
    ...
    public function __construct()
    {
        ...
        // $this->middleware('guest:school_admin')->except('logout');
        $this->middleware('guest:school_api')->except('logout');
    }
    ...
    public function getOTP(Request $request)
    {
        ...
        if (($user) && (Hash::check($request->input('password'),$user->password))){
            ...
            $otp = OtpGenerator::generateAndSaveOtp($user->username, 'school');
    ...
    public function login(Request $request)
    {
        ...
        $status = OtpGenerator::confirmOTP($request->username, $request->otp, 'school');

        if($status==1){
            if (Auth::guard('school_admin')->attempt(['username' => $request->username, 'password' => $request->password],true)) {
    ...
    {
        Auth::guard('school_admin')->logout();
    ...
    protected function createNewToken($token){
        return response()->json([
            'access_token' => $token,
            'token_type' => 'bearer',
            'expires_in' => auth('school_api')->factory()->getTTL() * 60,
            'user' => auth('school_api')->user(),
        ]);
    }

    public function refresh() {
        return $this->createNewToken(auth('school_api')->refresh());
    }

    public function api_login(Request $request)
    {
        
        if(! $token = Auth::guard('school_api')->attempt(['username' => $request->username, 'password' => $request->password])) 
        {
            return response()->json([
                'error' => 'Unauthorized',
