...
class ForgetPassword extends Controller
{
    public function forget(Request $request)
    {
        if($request->isMethod('post')) {
            $user =  Admin::where(['username' => $request->username, 'is_aeozeo' => false])->first();
           ...
           if(is_null($user->email)) {
            ...
            $link = route('ministry_reset_password').'?email='.$email.'&token='.$token.'&expire='.$expire_date;
        ...
        return view('ministry.auth.forget-password');
    }
    ...
    public function reset(Request $request)
    {
        ...
        if ( ! $validation->passes()) {
            return redirect()->route('ministry_forget_password')->withErrors($validation);
        }

        $user =  Admin::whereEmail($request->email)->first();

        if(is_null($user)) {
            return redirect()->route('ministry_forget_password')->withErrors('User not found');
        }

        $decrypted = Encryption::decrypter($request->token);

        if ($user->fullname != $decrypted){
            return redirect()->route('ministry_forget_password')->withErrors("Invalid token, please try again");
        }
        ...
        if ($now > $expire_date){
            return redirect()->route('ministry_forget_password')->withErrors("Link expired, please try again");
        }

        return view('ministry.auth.reset-password')->with([
            'username' => $user->username,
            'email' => $user->email,
